...
export async function insertTenders(
  ...
  const sql = `INSERT OR IGNORE INTO tenders
    (tender_id,title,agency,serial_no,bidding_method,amount_text,amount_value,tender_type,start_date,end_date,award_status,award_amount_text,award_amount_value,winning_vendor,source_url,source_hash,fetched_date)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
  ...
  for (const t of tenders) {
    try {
      const result = await db.prepare(sql).bind(
        ...
        t.winningVendor, t.sourceUrl, t.sourceHash, fetchedDate
      ).run();
